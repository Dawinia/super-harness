...
the lifecycle still works — you just drive it by hand with `super-harness
change start` / `plan ready` / `done` instead of having artifacts on disk do
it for you.
...
- **`super-harness status` shows no events even after I created proposal /
  ...
  to mark the plan ready manually. If you want automatic event emission from
  on-disk artifacts, switch to a framework adapter (`adapter install
  openspec`).
...
- **AGENTS.md shows the Plain block even after I installed OpenSpec.** The
  framework slot is single-occupancy — installing `openspec` replaces the
  Plain block with the OpenSpec block. If you still see Plain, re-run
  `adapter install openspec` (idempotent re-render).
...
`super-harness change start` / `plan ready` / `done` still work because they
are CLI surface, not adapter surface. Uninstall only removes the documented
...
- [`docs/getting-started.md`](../getting-started.md) — the end-to-end
  ...
  is installed.
- [`docs/cli-reference.md`](../cli-reference.md) — the manual lifecycle
  commands you use under Plain: `change start`, `plan ready`, `done`.
