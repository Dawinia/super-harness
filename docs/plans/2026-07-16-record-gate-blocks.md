...
- Modify: `tests/unit/core/test_paths.py` (**exists** — the `gate_blocks_path` test is added here, not a new file)
...
- Modify: `private/OPEN-ITEMS.md` (register Stage-2 done + the killed tier line)
- This plan doc: `docs/plans/2026-07-16-record-gate-blocks.md`

> **Close-out artifacts NOT in review scope:** `private/CAPABILITY-CONVERGENCE-LEDGER.md`/`.html` and `private/NEXT-SESSION-PROMPT.md` are refreshed *post-merge* (they reference the merge commit) and are gitignored, so they never appear in this PR's reviewable diff. They are handled in Phase F step 5, outside the attested change diff.

---

## Phase 0: Lifecycle setup + plan approval — BEFORE any implementation (addresses CODX-001)

The live pre-tool-use gate blocks source edits in `INTENT_DECLARED` / `AWAITING_PLAN_REVIEW` / `PLAN_REJECTED`; edits are only gate-ALLOWed in `PLAN_APPROVED` / `IMPLEMENTATION_IN_PROGRESS` / `CODE_REVIEW_REJECTED`. So the whole implementation (Tasks 1–6) MUST come AFTER plan approval + `implementation start`. Order:

1. `super-harness change start record-gate-blocks` → `INTENT_DECLARED`.
2. Draft THIS plan (author in a gated state via shell, not the Edit tool).
3. `super-harness plan ready record-gate-blocks --tier-hint Normal --scope @<scope.yaml listing every Scope file above>` → `AWAITING_PLAN_REVIEW`.
...
Only now do Tasks 1–6 run. Each task is TDD (write failing test → run red → implement → run green → commit).
...
**Files:** Modify `src/super_harness/engineering/gitignore_injector.py` (`_CANONICAL_PATHS`, add `.harness/gate-blocks.jsonl` next to `events.jsonl`; extend group-1 comment) + `tests/unit/engineering/test_gitignore_injector.py` (assert it is in `_CANONICAL_PATHS` and the rendered block ignores a realistic path).
...
**Tests (red):**
- `test_edits_blocked_counts_distinct_target_tuples` — raw records (c1,a.py,S)x2 + (c1,b.py,S)x1 → edits_blocked == 2 (retry duplicate collapses; two distinct targets counted). Assert the docstring/semantics are "distinct (change,file,state)", not raw.
...
**Implement:** add Band-1 field `edits_blocked: int` (NO default → every construction must supply it; update existing test constructions). `_edits_blocked(records, lo, hi)` → size of the dedup set of `(change_id, file, state)` after the same window filter as `_in_window` (unparseable ts excluded when a bound is set). Docstring states plainly: **distinct out-of-lifecycle edit targets; a conservative floor that under-counts (retries collapse), never inflates.** Wire into `build_value_report` via `read_blocks(gate_blocks_path(workspace_root))` with the already-end-of-day-extended `hi`.

**Commit** `feat(report): count distinct out-of-lifecycle edit targets the gate held`

---

## Task 6: Render honestly + retire the Stage-2 footnote debt in `cli/report.py`

**Files:** Modify `src/super_harness/cli/report.py`; Test `tests/unit/cli/test_report.py`.
...
**Implement:**
- "Caught for you": add `- {r.edits_blocked} distinct out-of-lifecycle edit target(s) the gate held (file x state; a conservative floor — retries collapse)`.
- Footnote: drop the lifecycle gate from the still-invisible list (its blocks are now traced); keep locked rules + verification + doc-sync as "a further Stage 2 cut".
- `_bottom_line`: the no-catch guard also requires `edits_blocked == 0`; add clause `the gate held {N} out-of-lifecycle edit target(s)` when >0. Honest framing — a held target, never "prevented N disasters".
- `--json` unchanged (`asdict` includes the field).

**Commit** `feat(report): surface gate-held targets + retire the Stage-2 no-trace footnote`
...
4. **Code review, 2 independent sources (codex + claude) at FULL intensity** — this touches the gate hot path; the Task-4 `test_block_still_blocks_when_recording_fails` safety property is the headline reviewers must confirm. On reject, delta round = `review prepare` (fix stays in-scope).
...
3. **Honest metric.** The headline is the distinct `(change_id, file, state)` target floor — a deliberate conservative UNDER-count (no content recorded → retries collapse), rendered as "held targets", never "N edits" or "prevented N disasters".
