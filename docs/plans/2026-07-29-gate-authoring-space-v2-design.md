...
skeleton ships enabled patterns for both superpowers candidate dirs.
...
- **Derived docs.** No new CLI surface: `change start --plan` was rejected (D2 — the
  value would be supplied by the governed agent), so `docs/cli-reference.md` is
  untouched. `sync --agents-md` must still re-render the injected section so the
  agent is finally told about the authoring space.
