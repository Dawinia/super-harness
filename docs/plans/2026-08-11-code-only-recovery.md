...
**`budget_holds` stays a separate `--json` field, and its stated reason has to be
rewritten.** `cli/attest.py:201-206` justifies keeping `review_budget_rounds_held` off the
independence item on two grounds, and Cut 3 removes one of them: independence is no longer
"scoped to code review by design §4.1". The surviving ground is the other half of the same
comment and is enough alone — `derive_independence` counts every `review_budget_exceeded`
**whatever role raised it**, so the figure is per-change and belongs to no single role's
row. Folding it onto one would re-open the misattribution GitHub #96 closed, which is what
an implementer would do on finding the comment's premise gone. Rewrite the comment to lead
with the role-agnostic reason; do not move the field. `docs/getting-started.md:376-377`
states the removed premise in prose and changes with it.
...
Cut 3 makes a second set of sentences wrong rather than merely incomplete. Both agent
adapters (`adapters/agent/claude_code.py:132`, `adapters/agent/codex.py:119`, rendered to
`AGENTS.md:104`) and `docs/getting-started.md:460-461` say it is a *code-review* skip that
needs `--override --reason` to pass attestation. After Cut 3 that is true of a plan-review
skip too, and an agent following the narrower text hits a wedged plan reviewer, skips it
bare, reaches `READY_TO_MERGE`, and fails `attest verify` in CI on a blocker its own
instructions said did not apply to it. All four files widen to name both roles, and the
sentence is pinned by a test so the two adapters cannot drift apart or be left behind.

`docs/cli-reference.md` is derived and is regenerated, not hand-edited. `AGENTS.md` is
regenerated by `sync --agents-md`, and `doc check` fails if it is stale.
...
- `attest verify --json` still carries `budget_holds` as its own field, not folded onto an
  independence item — the #96 regression, now that its original rationale is gone.
- The rendered agent guidance names both roles in the skip-attestation sentence, asserted
  once so neither adapter can drift from the other or be left on the code-only wording.
...
(status → `retired`), `docs/decisions/d-single-gate-policy.md` (tier-2 reconcile: it
anchors `gates/decisions.py`, whose `SUGGESTIONS` this change edits, so its
`reconciled_anchors` hash re-stamps and the file itself changes),
`docs/plans/2026-08-11-code-only-recovery.md`,
...
`tests/unit/engineering/test_agents_md_render.py`, `tests/unit/daemon/test_hook_entry.py`,
