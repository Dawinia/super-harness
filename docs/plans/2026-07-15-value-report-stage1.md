...
- `rejected_rounds` — `review_round_closed` events whose `payload.outcome == "rejected"` (a reviewer actually returned blocking work). Count ONLY `rejected`, NOT `execution_failed`: that outcome is a catch-all for target-stale, incomplete-governance, missing imports, and the NORMAL mixed automated+human path where automated evidence passed but the human review is still pending (review.py:1360-1395) — lumping those into "rework" fabricates cost and breaks the honesty law (CODX-006). This deliberately under-counts genuine infra-crash rework (CODX-005 raised the opposite concern), but a precise label that never fabricates beats a broad one that does. The closed-round vocabulary is exactly `approved`/`rejected`/`execution_failed` (there is NO `failed`); the imported verdict schema has NO `outcome` field — rejection lives on the round-closed milestone, not the verdict (CODX-001).
...
Tokens live in each `review_result_imported` receipt: `payload.receipt.usage` (often `None`). **Rejected rounds live on `review_round_closed.payload.outcome == "rejected"`** — count ONLY `rejected`. Do NOT count `execution_failed`: it conflates infra-stale / incomplete-governance / the normal human-quorum-pending path with genuine failure, so counting it fabricates rework (CODX-006). Closed-round vocabulary is exactly `approved`/`rejected`/`execution_failed` (no `failed`); the imported verdict has no `outcome` field (CODX-001).
...
def test_review_tokens_usage_and_rejected_rounds(tmp_path):
    events_file = _write_events(tmp_path, [
        ...
        _round_closed("e6", "c1", "2026-07-02T05:00:00Z", "execution_failed"),  # does NOT count (ambiguous: infra-fail vs normal human-quorum-pending, CODX-006)
    ])
    report = build_value_report(events_file, since=None, until=None, workspace_root=tmp_path)
...
- **No phantom signals** — `scope_drift_detected` is never referenced; `rejected_rounds` counts ONLY `review_round_closed.payload.outcome == "rejected"`, NOT the ambiguous `execution_failed` (which fabricates rework — CODX-006), and not the verdict (CODX-001).
