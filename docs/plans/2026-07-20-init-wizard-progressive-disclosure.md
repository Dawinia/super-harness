...
- [ ] Add failing adapter tests proving checkbox/select prompts pass
...
- [ ] Pass `erase_when_done=True` through Questionary's application kwargs for
...
- [ ] Add failing renderer/UI tests for these completed lines:
...
- [ ] Run the answer-summary tests and confirm RED.
- [ ] Add a library-neutral `render_answer(label, value)` boundary to
...
- [ ] Run the answer-summary tests and confirm GREEN.
...
- [ ] Add failing tests proving `create_init_ui` receives the global verbose count
...
- [ ] Add paired default/verbose integration tests proving the same scripted
...
- [ ] Run the focused review/verbosity tests and confirm RED.
- [ ] Thread `bool(ctx.obj.get("verbose"))` through `create_init_ui` into
...
- [ ] Add failing tests for one default line per successful public group:
...
- [ ] Add failing success/failure/cancel/interruption tests proving the renderer
...
- [ ] Run the focused event/outcome tests and confirm RED.
- [ ] Map succeeded executor events into stable public outcome groups, deduplicate
...
- [ ] Add the pre-change representative transcript as a named checked-in test
...
- [ ] Run guided compatibility slices:
...
- [ ] Update `docs/getting-started.md` to state that guided mode hides unchanged
  detail by default and that `super-harness --verbose init` expands the review and
  apply diagnostics.
- [ ] Run decision/doc/static/full verification:
...
- [ ] Manually exercise the same two-agent fixture in a wide color terminal and a
