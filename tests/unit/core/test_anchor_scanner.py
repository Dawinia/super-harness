"""Tests for the pure, parametrized `<keyword><id>` sentinel scanner.

The scanner takes a required ``keyword`` and collects every ``<keyword><id>``
sentinel found beneath a workspace root. These tests exercise the pure function
directly with synthetic fixtures; the live callers (`decision check` /
`decision show`) pass ``keyword="@decision:"``.
"""
...
def test_scan_finds_capability_sentinels(tmp_path: Path) -> None:
    ...
    found = scan_sentinels(tmp_path, file_globs=["**/*"], keyword="@capability:")
...
def test_scan_works_outside_git_repo(tmp_path: Path) -> None:
    ...
    found = scan_sentinels(tmp_path, keyword="@capability:")
...
def test_scan_honors_specific_glob_filter(tmp_path: Path) -> None:
    ...
    found = scan_sentinels(tmp_path, file_globs=["*.py"], keyword="@capability:")
...
def test_locations_returns_file_and_1based_line(tmp_path: Path) -> None:
    ...
    locs = scan_sentinel_locations(tmp_path, keyword="@capability:")
...
def test_keyword_param_scans_decision(tmp_path: Path) -> None:
    ...
    # a different keyword selects a different namespace:
    assert scan_sentinels(tmp_path, keyword="@capability:") == {"cap-foo"}
