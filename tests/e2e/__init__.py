...
These tests drive the installed ``super-harness`` / ``super-harness-hook``
binaries by name (which MUST be resolvable on PATH) so the full
install → hook (in-process gate) → on-merge → L1-updater path is proven
